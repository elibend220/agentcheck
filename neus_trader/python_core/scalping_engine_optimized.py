"""
Optimized Golden Hour Scalping Engine
Phase 2: Market hour filtering + improved order flow

Combines:
- Improved order flow analyzer with 2+ confirmations
- RSI filtering (20-80 range)
- Market hour filtering (London + US Close only - 65% and 60% win rates)
- Position sizing scaled by signal quality

Target: Achieve Profit Factor ≥ 1.5
Current: PF 1.28 with prime hours filtering
"""
...
class GoldenHourScalpingEngineOptimized:
    """
    OPTIMIZED: Adaptive scalping engine with market hour filtering.
    Only trades during high-liquidity windows (London + US Close).
    Target: PF ≥ 1.5
    """
    ...
    def __init__(
        ...
        prime_hours_only: bool = True  # NEW: Filter to prime hours
    ):
        self.logger = logging.getLogger("ScalpingEngineOptimized")
        self.symbol = symbol
        self.session_id = session_id
        self.min_confidence = min_confidence
        self.prime_hours_only = prime_hours_only
        ...
        self.trade_logger = TradeLogger(fallback_file="/tmp/neus_trades_optimized.jsonl")
        ...
        self.prime_hour_trades = 0

        self.logger.info(
            f"ScalpingEngineOptimized initialized: {symbol}, Capital=${initial_capital}, "
            f"Session={session_id}, MinConfidence={min_confidence}, PrimeHoursOnly={prime_hours_only}"
        )
    ...
    def on_candle(
        ...
        Only generates signals during prime trading hours.
        """
        try:
            ...
            # NEW: Check if in prime trading hours
            if self.prime_hours_only:
                is_prime = self.market_hours.is_prime_trading_hour(timestamp)
                if not is_prime:
                    return None

            # Get market state
            market_state = self.market_hours.get_market_state(timestamp) if hasattr(self.market_hours, 'get_market_state') else {
                'is_trading_active': True,
                'is_prime_hour': self.market_hours.is_prime_trading_hour(timestamp),
                'session': self.market_hours.get_current_session(timestamp).value,
            ...
            if scalping_signal:
                self.logger.info(
                    f"✓ PRIME HOUR Signal: {scalping_signal.direction} "
                ...
                self.event_buffer.enqueue_signal(
                    ...
                    agent_id="scalping_engine_optimized",
                    session_id=self.session_id,
                    metadata={
                        'market_session': market_state.get('session', 'unknown'),
    ...
    def _create_scalping_signal(
        ...
        # OPTIMIZED: Adjust position size based on signal quality
        if order_flow_signal.signal_quality == "HIGH":
        ...
        # Adjust for prime market conditions
        if market_state.get('is_prime_hour', False):
            position_size *= 1.0
        else:
            position_size *= 0.6
        ...
        return ScalpingSignal(
            ...
            signal_sources=['order_flow_improved', 'market_hours', 'adaptive_risk'],
    ...
    def _update_monitoring(self, current_price: float, timestamp: datetime):
        ...
        metrics['prime_hour_trades'] = self.prime_hour_trades
        self.monitor.update_metrics(metrics)

    def get_system_status(self) -> Dict:
        """Get comprehensive system status"""
        market_state = self.market_hours.get_market_state() if hasattr(self.market_hours, 'get_market_state') else {}
        ...
        return {
            ...
            'prime_hour_trades': self.prime_hour_trades,
            'prime_hours_only': self.prime_hours_only,
    ...
    def start(self):
        ...
        self.logger.info(f"Optimized scalping engine started (prime_hours_only={self.prime_hours_only})")

    def stop(self):
        """Stop the scalping engine"""
        self.is_running = False
        self.ledger_worker.stop()
        self.logger.info(f"Optimized scalping engine stopped")
